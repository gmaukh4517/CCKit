...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [UIApplication sharedApplication].statusBarStyle = PhotoiOS7Later ? UIStatusBarStyleLightContent : UIStatusBarStyleBlackOpaque;
}
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated {
    if (PhotoiOS7Later) viewController.automaticallyAdjustsScrollViewInsets = NO;
...
+ (UIImage *)imageNamedFromMyBundle:(NSString *)name {
    UIImage *image = [UIImage imageNamed:[@"CCKit.bundle" stringByAppendingPathComponent:name]];
    if (image) {
        return image;
    } else {
        image = [UIImage imageNamed:[@"Frameworks/CCKit.bundle" stringByAppendingPathComponent:name]];
        if (!image) {
