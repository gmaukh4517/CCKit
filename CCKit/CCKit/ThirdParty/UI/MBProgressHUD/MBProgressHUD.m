...
- (void)commonInit
{
    ...
    _mode = MBProgressHUDModeIndeterminateLogo;
    _coveredNavigationBar = YES;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)updateConstraints
{
    ...
    CGFloat constantY = offset.y;
    if (self.coveredNavigationBar)
        constantY = offset.y - 32;
    
    NSMutableArray *centeringConstraints = [NSMutableArray array];
    [centeringConstraints addObject:[NSLayoutConstraint constraintWithItem:bezel attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeCenterX multiplier:1.f constant:offset.x]];
    [centeringConstraints addObject:[NSLayoutConstraint constraintWithItem:bezel attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeCenterY multiplier:1.f constant:constantY]];
...
- (void)setCoveredNavigationBar:(BOOL)coveredNavigationBar
{
    if (coveredNavigationBar != _coveredNavigationBar) {
        _coveredNavigationBar = coveredNavigationBar;
        [self setNeedsUpdateConstraints];
    }
}
    ...
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            ...
            if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
                _color = [UIColor colorWithWhite:0.0f alpha:0.4f];
            } else {
