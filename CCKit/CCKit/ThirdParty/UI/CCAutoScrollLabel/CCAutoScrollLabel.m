...
static void autoScroll_each_object(NSArray *objects, void (^block)(id object))
{
...
#define AUTOSCROLL_LABEL(ATTR, VALUE) autoScroll_each_object(self.labels, ^(UILabel * label) { label.ATTR = VALUE; });

@interface CCAutoScrollLabel ()
...
- (void)commonInit
{
    ...
    // default values
    _scrollDirection = CCAutoScrollDirectionLeft;
...
- (void)setAttributedText:(NSAttributedString *)theText refreshLabels:(BOOL)refresh
{
    ...
    AUTOSCROLL_LABEL(attributedText, theText)
...
{
    AUTOSCROLL_LABEL(textColor, color)
}
...
- (void)setScrollDirection:(CCAutoScrollDirection)direction
{
...
{
    AUTOSCROLL_LABEL(shadowColor, color)
}
...
{
    AUTOSCROLL_LABEL(shadowOffset, offset)
}
...
- (void)scrollLabelIfNeeded
{
    ...
    BOOL doScrollLeft = (self.scrollDirection == CCAutoScrollDirectionLeft);
...
- (void)refreshLabels
{
    ...
    autoScroll_each_object(self.labels, ^(UILabel *label) {
        [label sizeToFit];
    ...
    if (CGRectGetWidth(self.mainLabel.bounds) > CGRectGetWidth(self.bounds)) {
        ...
        AUTOSCROLL_LABEL(hidden, NO)
        
        [self applyGradientMaskForFadeLength:self.fadeLength
                                  enableFade:self.scrolling];
        
        [self scrollLabelIfNeeded];
    } else {
        // Hide the other labels
        AUTOSCROLL_LABEL(hidden, (self.mainLabel != label))
...
- (void)applyGradientMaskForFadeLength:(CGFloat)fadeLength enableFade:(BOOL)fade
{
    ...
    if (fadeLength) {
        ...
        if (!fade) switch (self.scrollDirection) {
            case CCAutoScrollDirectionLeft:
                leftFadePoint = @0;
                break;
                
            case CCAutoScrollDirectionRight:
                leftFadePoint = @0;
                rightFadePoint = @1;
                break;
        }
